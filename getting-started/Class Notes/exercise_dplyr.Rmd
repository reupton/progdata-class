...
```{r}
select(gapminder, FILL_THIS_IN) ## select pulls out columns of a data set
```

2. Select all variables, from `country` to `lifeExp`.

- select() selects from data frame
...
# Do it shorter with `:` :
select(gapminder, country : lifeExp) ## selects everything from one column to another (everything in between)
```

3. Select all variables, except `lifeExp`.

```{r}
select(gapminder, - lifeExp) ## can also use a ! but better to use -
```
...
select(gapminder, Continent = continent) # makes continent the first and only column

rename(gapminder, Continent = continent) # does not move column
```
...
```{r}
arrange(gapminder, year) # puts the data in order by year
```
...
```{r}
gapminder |>
  filter(country == "China" | country == "Brazil" & pop > 1000000)

gapminder |>
  filter(country %in% c("China", "Brazil") & pop > 1000000)
```
...
gapminder |>
  mutate(
    lifeExp = if_else(country == "Canada" & year == 1952,
                      true = 70,
                      false = lifeExp)
  )  ## if country == canada and year == 1952 is true, change the value of life expectancy to 70, if it is false, change the value of life expectancy to the value of life expectancy (leave as is basically)
```
